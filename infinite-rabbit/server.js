...
// Note: ousia-web3/game_test used sqlite3. 
// For maximum compatibility and ease of deployment for the user, 
// I'll start with a json-based leaderboard unless sqlite3 is confirmed available.
// However, the reference used sqlite3, so I'll provide a compatible structure.
...
// Initialize simple JSON DB (Replacing SQLite for easier setup without binary dependencies)
let scores = [];
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === "/api/scores" && req.method === "POST") {
      ...
      scores.push(r);
      // Keep only top 100 in file to avoid bloat
...
function normalizeScore(payload) {
  const nickname = String(payload.nickname || "").trim().slice(0, 12) || "익명 토끼";
...
function serveStatic(pathname, res) {
  ...
  if (!fs.existsSync(targetPath) || fs.statSync(targetPath).isDirectory()) {
    // If resource not found in public, try falling back to index.html (SPA style) 
    // but here we just return 404 for simplicity as it's not a SPA.
